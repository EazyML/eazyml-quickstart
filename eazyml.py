...
def ez_load(auth_token, filename, options = None):
    ...
    return_response: type 'dict'
        ...
        Where:
            ...
            dataset_id: Uploaded file id which would be used in further calls if successful else set to None
    """
    status_code = 500
    try:
...
def ez_types(auth_token, dataset_id, options = None):
    ...
    auth_token : type 'str'
        The auth token generated by ez_auth function
    dataset_id : type 'str'
        The upload dataset ID for which inferred/updated types needs to be returned
    ...
    return_response: type 'dict'
        ...
        Where:
            ...
            ez_dtypes: Returns pandas dataframe for supplied dataset id columns and its dtypes if call successful
                          else set to None               
    """
    status_code = 500
    try:
...
def ez_outlier(auth_token, dataset_id, options = None):
    ...
    return_response: type 'dict'
        ...
        Where:
            success     : True if call successful else False
            outliers : Returns pandas dataframe for supplied dataset id outliers detected if call successful
                          else set to None               
    """
    status_code = 500
    try:
...
def ez_impute(auth_token, dataset_id, options = None):
    ...
    return_response: type 'dict'
        ...
        Where:
            success     : True if call successful else False
            imputations : Returns pandas dataframe for supplied dataset id with imputed values if call successful
                          else set to None               
    """
    status_code = 500
    try:
...
def ez_shuffle(auth_token, dataset_id, options = None):
    ...
    options    : type 'dict'
        ...
        Where:
        shuffle               : Set "yes" if user wants to shuffle the rows in the training data else set "no". Default value is "yes".
    Returns
    -------
    return_response: type 'dict'
        {
...
def ez_init_model(auth_token, dataset_id, options = None):
    ...
    options : type 'dict'
        ...
        Where:
        accelerate       : Set "yes" if user wants machine to upload the data without .... user else set "no"
        model_type            : Set to predictive to build predictive model else build time series model
        date_time_column      : Set None if predictive model else specify date time column for time series
        remove_dependent : Set "yes" if user want to remove dependent predictors else set "no"
        derive_numeric   : Set "yes" if user want to derive numeric predictors for model building else set "no" 
        ...
        text_types       : The user can specify the different text types that he wants to derive.
                           ...
                           The "*" denotes all other column names. Suppose there are 3 text columns col1, col2, col3,
                           then in this  example, for col1 we will derive sentiments and for all the other columns we derive
                           the 4 types mentioned.
        phrases          : The phrases to be used for concept extraction. Only to be specified for those columns for which concept
                           extraction is needed. 
                           Ex: {"col1":["phrase1", "phrase2"], "*":["phrase3", "phrase4"]}
                           The "*" denotes all other column names. Suppose there are 3 text columns col1, col2, col3,
                           then in this  example, for col1 we will use phrases "phrase1", "phrase2" in case the user specifies
                           concept extraction as a type to be derived in text_types above and for all the other columns we use
                           phrases "phrase3", "phrase4". This parameter will only come into use if the user specifies 
                           concept extraction as a test type for any column.
    ...
    return_response: type 'dict'
        ...
        Where:
            success              : True if call successful else False
            model_id             : model_id created for the dataset if call successful
                                   else set to None               
...
def ez_derive_numeric(auth_token, model_id, options = None):
    ...
    options : type 'dict'
        ...
        return_dataset       : Set "yes" if the user wants to return the dataframe with the added columns. If both are set to 
                               "yes", then only columns are returned
        expressions          : Set None if user do not want to specify numeric derivations, machine will derive on its own
                               if derive_numeric set to "yes". If user want to derive numeric predictors as per their need
                               user should specify it.
                               Expressions should be of the format '"Column_1" (operator) "Column_2"' where operator
    ...
    return_response: type 'dict'
        ...
        Where:
            success            : True if call successful else False
            dataframe : Pandas dataframe after deriving numeric predictors if return_dataset set to "yes"
                                 else set to None
...
def ez_derive_text(auth_token, model_id, options = None):
    ...
    Note
    -----
    Calling this function would remove the text columns from the dataset, so calling this function twice on the same dataset ID and model ID
...
def ez_select_features(auth_token, model_id, options = None):
    ...
    return_response: type 'dict'
        ...
        Where:
            success           : True if call successful else False
            features          : list of features selected
    """
    status_code = 500
    try:
...
def ez_build_ensemble(auth_token, model_id, ensemble_constituents, options = None):
    ...
    Optional Parameters
    -------------------
    ensemble_constituents_weights: type: list of integers
    Should contain integers specifying the weights of each model included in the ensemble
...
def ez_predict(auth_token, model_id, filename, options = None):
    ...
    model_id : type 'str'
        The model_id created by init_model
    *model_name : type 'str'
        The model name which should be used for prediction on prediction dataset file. 
    ...
    return_response: type 'dict'
        ...
        Where:
            success         : True if call successful else False
            prediction_dataset_id : ID created for uploaded prediction dataset if call successful
                                   else set to None               
            predictions    : Dataframe contains all the prediction dataset rows with an 
                              additional column of predicted values. The additional 
                              column name would be "predicted_<outcome_column name>"
    """
    status_code = 500
    try:
...
def ez_explain(auth_token, model_id, prediction_dataset_id, options = None):
    ...
    prediction_dataset_id : type 'str'
        The prediction dataset ID which should be used for explaining the prediction
    record_number : type 'num'
        ...
        Can also be a comma separated string if needed to fetch multiple explanations
    ...
    return_response: type 'dict'
        ...
        Where:
            success         : True if call successful else False
            prediction_dataset_id : Returns explanation of the record number. The explanation dictionary contains:
                              {
                                 'prediction_dataset_id': <prediction_datset_id>, 
                                 'explanation': <explanation sentence for the predicted value of the record_number>,
                                 'prediction': <predicted_value for the record_number>,
                                 'local_importance': <Importance Dataframe with column name in order of importance>
                               }
...
def ez_insert(auth_token, filename, dataset_id, options = None):
    """EazyML Insert function. Override EazyML data preprocessing steps using this function.

       Usage: Let your function do some steps using data preprocessing EazyML functions.
              Then you can call the fetch function to get the pre processed data from EazyML
              then perform your own preprocessing steps. Now in order to let EazyML know 
              of this preprocessing, you should use this function, giving the preprocessed
              dataframe in either csv/xlsx/xls format, followed by dataset_id and if applicable,
              model_id
...
def ez_fetch(auth_token, dataset_id, options = None):
    ...
    return_response: type 'dict'
        ...
        Where:
            success              : True if call successful else False
            dataframe            : A dictionary with the keys columns, index and data
            columns              : list of columns
    """
    status_code = 500
    try:
...
def ez_config(auth_token, filename, options = None):
    """EazyML Configuration function.
       ...
       For example:
           In the remove dependent predictors, You can increase the threshold used by VIF
           by making an ini file with VIF_THRESHOLD = 100 for example
    ...
    auth_token : type 'str'
        The auth token generated by ez_auth function
    filename: type 'str'
        The path to valid config file, which is of ini format. The config file should have section 'UPLOAD_DATA and if model_id is passed then it should have 
        section 'BUILD_MODEL' and if prediction_dataset_id is passed, it should have 'TEST_MODEL' section.
    options : type 'dict'
...
def ez_get_model_status(auth_token, model_id, options = None):
    """EazyML model status fetch function
       ...
       Usage: get_model_status function gets the status of a model_id from EazyML
...
def ez_get_datasets(auth_token, options = None):
    """EazyML training datasets fetch function
       ...
       Usage: ez_get_datasets function gets all of your training datasets from EazyML
...
def ez_get_test_datasets(auth_token, options = None):
    """EazyML test datasets fetch function
       ...
       Usage: ez_get_test_datasets function gets all of your test datasets from EazyML
...
def ez_get_models(auth_token, options = None):
    """EazyML models fetch function
       ...
       Usage: ez_get_models function gets all of your models from EazyML
        ...
        try:
            ...
            for chunk in response.iter_content(chunk_size=512):
                if chunk:
                    handle.write(chunk)
            handle.close()
            return {'message': "The model has been exported to a file with name '{0}' and is downloaded successfully".format(filename), 'status code' : status_code}
        except Exception as e:
            response.raise_for_status()
    except Exception as e:
        traceback.print_exc()
