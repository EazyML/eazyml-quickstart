...
def ez_impute(auth_token, dataset_id, options=None):
    ...
    return_response : type 'dict'
        ...
        Where:
            success     : True if call successful else False
            imputations : Returns pandas dataframe for supplied dataset id with
                          imputed values if call successful else set to None
    """
    status_code = 500
    try:
...
def ez_shuffle(auth_token, dataset_id, options=None):
    ...
    auth_token : type 'str'
        The auth token generated by ez_auth function
    dataset_id : type 'str'
        The dataset ID for which shuffling needs to be done.
    options    : type 'dict'
        A Dictionary of format
        options = {
            "shuffle" : <"yes"|"no">
        ...
        Where:
        shuffle : Set "yes" if user wants to shuffle the rows in the training
                  data else set "no". Default value is "yes".
    Returns
    -------
    return_response : type 'dict'
        {
...
def ez_config(auth_token, filename, options=None):
    """EazyML Configuration function.
       ...
       For example:
           ...
           used by VIF by making an ini file with VIF_THRESHOLD = 100 for
           example and passing the ini file, dataset_id and model_id in
           this function
    ...
    filename   : type 'str'
        ...
        should have section 'BUILD_MODEL' and if prediction_dataset_id is
        passed, it should have 'TEST_MODEL' section.
    options    : type 'dict'
...
def ez_get_model_status(auth_token, model_id, options=None):
    """EazyML model status fetch function
       ...
       Usage : get_model_status function gets the status of a model_id
               from EazyML
...
def ez_get_datasets(auth_token, options=None):
    """EazyML training datasets fetch function
       ...
       Usage : ez_get_datasets function gets all of your training
              datasets from EazyML
    ...
    return_response : type 'dict'
        ...
        Where:
            success    : True if call successful else False
            message    : Upload status message of training datasets.
...
def ez_get_test_datasets(auth_token, options=None):
    """EazyML test datasets fetch function
       ...
       Usage : ez_get_test_datasets function gets all of your test
              datasets from EazyML
    ...
    return_response : type 'dict'
        ...
        Where:
            success   : True if call successful else False
            message   : Upload status message of test datasets.
            dataframe : A dictionary of the datasets
    """
    status_code = 500
    try:
...
def ez_delete_datasets(auth_token, dataset_id):
    """EazyML training datasets delete function
       ...
       Usage : ez_delete_datasets function deletes training datasets from EazyML
    ...
    return_response : type 'dict'
        ...
        Where:
            success : True if call successful else False
            message : Delete status messagei of training dataset.
    """
    status_code = 500
    try:
...
def ez_delete_test_datasets(auth_token, prediction_dataset_id):
    """EazyML training test datasets delete function
       ...
       Usage : ez_delete_test_datasets function deletes prediction
               datasets from EazyML
    ...
    return_response : type 'dict'
        ...
        Where:
            success : True if call successful else False
            message : Delete status message of test datasets.
    """
    status_code = 500
    try:
...
def ez_delete_models(auth_token, model_id):
    ...
    return_response : type 'dict'
        ...
        Where:
            success : True if call successful else False
            message : Delete status message of models.
    """
    status_code = 500
    try:
        ...
        try:
            ...
            for chunk in response.iter_content(chunk_size=512):
                if chunk:
                    handle.write(chunk)
            handle.close()
            return {'message': "The model has been exported to a file with\
                                   name '{0}' and is downloaded\
...
def ez_validate(auth_token, model_id, filename, options=None):
        ...
        Where:
            ...
            validations : Returns validations of the record numbers.
                          The validation dictionary contains:
                {
                 ...
                 ...             : <other scores based on config params set
                                   like accuracy, coverage, perception, etc.>
                }
